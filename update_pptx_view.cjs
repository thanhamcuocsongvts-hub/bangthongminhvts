        ...
        {fileType === 'pptx' && (
          <div className="flex-1 w-full h-full overflow-y-auto p-6 flex flex-col items-center justify-center text-center bg-gradient-to-br from-orange-950/20 to-slate-950">
            <div className="relative p-10 rounded-3xl bg-slate-900 border border-orange-500/30 shadow-2xl max-w-2xl w-full flex flex-col items-center overflow-hidden">
              ...
              <h2 className="text-2xl font-black text-white mb-2 relative z-10">Chế Độ Trình Chiếu PowerPoint Chuyên Nghiệp</h2>
              <p className="text-slate-400 text-sm max-w-md mx-auto mb-8 relative z-10">
                Để đảm bảo 100% các hiệu ứng chuyển động (Animations), âm thanh và font chữ phức tạp được hiển thị chính xác như bản gốc, hệ thống yêu cầu mở bằng phần mềm PowerPoint trên máy.
              </p>
              ...
              <div className="w-full mt-6 pt-6 border-t border-slate-800/80 flex items-start gap-4 text-left relative z-10">
                ...
                <div>
                  <h4 className="text-sm font-bold text-slate-200 mb-1">Mẹo trình chiếu</h4>
                  <p className="text-xs text-slate-400 leading-relaxed">
                    Sau khi bấm nút màu cam, tệp sẽ được tải xuống. Thầy/Cô chỉ cần click mở tệp vừa tải ở góc dưới màn hình trình duyệt, phần mềm PowerPoint sẽ tự động bật lên sẵn sàng trình chiếu với đầy đủ hiệu ứng.
                  </p>
                </div>
              </div>
            </div>
...
// I need to replace the old {fileType === 'pptx' ... } block.
...
if (!code.includes('Info')) {
  code = code.replace(/import \{([\s\S]*?)FileText/, "import { Info, $1FileText");
}
