...
export const StudentConductModal: React.FC<StudentConductModalProps> = ({
  ...
  // Quick categories list matching school standards
      ...
      <div className="relative w-full max-w-xl bg-white rounded-3xl shadow-2xl border border-slate-200 overflow-hidden flex flex-col max-h-[92vh]">
        ...
        <div className="bg-[#1e3a8a] text-white p-5 flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h2 className="text-lg font-black tracking-tight">Ghi Nhận Vi Phạm / Khen Thưởng</h2>
              <p className="text-xs text-blue-200 font-medium">
                Người ghi nhận: <span className="font-bold text-white">{teacherName}</span> (Lớp {classroom.name})
              </p>
            </div>
          </div>
          <button
        ...
        <form onSubmit={handleSubmit} className="p-5 overflow-y-auto space-y-4 text-slate-800">
          ...
          <div>
            ...
            >
              {classroom.students.map((st, idx) => (
                <option key={st.id} value={st.id}>
                  STT {String(idx + 1).padStart(2, '0')} - {st.name} ({st.group || `Tổ ${((idx % 4) + 1)}`}) [Điểm thi đua: {st.bonusPoints >= 0 ? `+${st.bonusPoints}` : st.bonusPoints}]
                </option>
              ))}
            </select>
          </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            <div>
              ...
              <input
                ...
                className="w-full px-3 py-2 rounded-xl bg-slate-50 border border-slate-200 text-slate-900 text-sm font-black text-center focus:ring-2 focus:ring-blue-500 focus:outline-none"
              />
            </div>
