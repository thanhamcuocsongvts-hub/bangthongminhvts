                ...
                <div className="space-y-3">
                  <div className="flex items-center justify-between pb-2 border-b border-slate-100">
                    <div>
                      <div className="text-xs font-black text-slate-900 uppercase flex items-center gap-1.5">
                        ...
                        <span>Danh Sách Giáo Viên Đăng Ký ({teacherList.length})</span>
                      </div>
                      <p className="text-[11px] text-slate-500">Quản trị viên có thể xem mật khẩu, reset mật khẩu và quản lý tài khoản</p>
                    </div>
                    <button
                      type="button"
                  ...
                  <button
                    ...
                    onClick={() => {
                      ...
                      onSelectTeacher(adminAccount);
                      showToast('Đã đăng nhập vào phòng học với Quyền Quản Trị Viên (Admin)!');
                      if (onClose) setTimeout(onClose, 500);
                    }}
                    className="w-full py-2.5 rounded-xl bg-amber-500 hover:bg-amber-600 text-slate-950 font-black text-xs flex items-center justify-center gap-2 shadow-sm transition-all active:scale-98"
                  >
                          ...
                          <div className="flex items-center gap-2.5 overflow-hidden">
                            ...
                            <div className="overflow-hidden">
                              ...
                              <div className="text-[10px] text-slate-500 truncate">
                                User: <span className="font-mono text-slate-700 font-bold">{t.username || t.email}</span> • {t.subject}
                              </div>

                              {/* Password Display */}
                              <div className="flex items-center gap-1.5 mt-1">
                                <span className="text-[10px] font-bold text-slate-400">MK:</span>
                                <span className="font-mono text-[11px] font-bold text-indigo-700 bg-indigo-50/80 px-1.5 py-0.2 rounded border border-indigo-200/50">
                                  {isRevealed ? pwd : '••••••'}
                                </span>
                                <button
                                  type="button"
                                  onClick={() =>
                                    setRevealedPasswords((prev) => ({ ...prev, [t.id]: !prev[t.id] }))
                                  }
                          ...
                          <div className="flex items-center gap-1.5 shrink-0 self-end sm:self-center">
                            <button
                              ...
                              onClick={() => {
                                ...
                                showToast(`Đã reset mật khẩu tài khoản ${t.name} về mặc định 123456!`);
                              }}
                              className="px-2.5 py-1.5 rounded-lg bg-amber-500/10 hover:bg-amber-500/20 text-amber-900 border border-amber-300 font-bold text-[10px] transition-colors flex items-center gap-1"
                            ...
                            <button
                              ...
                              className="px-2.5 py-1.5 rounded-lg bg-indigo-50 hover:bg-indigo-100 text-indigo-700 border border-indigo-200 font-bold text-[10px] transition-colors"
